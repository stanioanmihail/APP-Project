...
#pragma omp parallel shared(delta_x, delta_y, rows, cols, magnitude) private(r, c, pos, sq1, sq2)
{
   tid = omp_get_thread_num();
   if (tid == 0)
      printf("magnitude_x_y with %d omp threads...\n", omp_get_num_threads());
...
#pragma omp parallel shared(smoothedim, rows, cols, delta_x, delta_y) private(tid, r, c, pos)
{
	tid = omp_get_thread_num();
   /****************************************************************************
   ...
   ****************************************************************************/
	if (tid == 0)
		if(VERBOSE) printf("   Computing the X-direction derivative with %d omp threads.\n", omp_get_num_threads());
...
#pragma omp parallel shared(image, rows, cols, tempim, smoothedim, center, kernel) private(tid, r, c, rr, cc, dot, sum)
{
   ...
   ****************************************************************************/
	if (tid == 0)
		if(VERBOSE) printf("   Bluring the image in the X-direction with %d omp threads.\n", omp_get_num_threads());
